...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  const rewardDistributor = await helper.get('GOVIRewardTrackerDistributor')

  await helper.deployProxy('GOVIRewardTracker', {
    args: [owner, 'Staked GOVI', 'stGOVI', [govi.address, esGovi.address], rewardDistributor.address, true, true],
  })
}
